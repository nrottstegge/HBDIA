#!/usr/bin/env python3
"""
Compare storage requirements across different sparse matrix formats.
...
def main():
    ...
    df['coo_storage_mb'] = df['coo_cost_bytes'] / (1024**2)
    df['csr_storage_mb'] = df['csr_cost_bytes'] / (1024**2)
    df['sell_storage_mb'] = df['sell_cost_bytes'] / (1024**2)
    df['hbdia_storage_mb'] = df['hbdia_cost_bytes'] / (1024**2)
    df['dia_storage_mb'] = df['dia_cost_bytes'] / (1024**2)
    ...
    formats = {
        'COO': ('coo_storage_mb', 'all'),
        'CSR': ('csr_storage_mb', 'all'), 
        'SELL': ('sell_storage_mb', 'all'),
        'DIA': ('dia_storage_mb', 'all'),
        'HBDIA-1': ('hbdia_storage_mb', 1),
        'HBDIA-8': ('hbdia_storage_mb', 8)
    }
    ...
    for format_name, (column, threshold) in formats.items():
        for noise in noise_values:
            if threshold == 'all':
                # For non-HBDIA formats, use all data regardless of threshold
                data = df[df['extracted_noise'] == noise][column]
            else:
                # For HBDIA formats, filter by specific threshold
                data = df[(df['extracted_noise'] == noise) & (df['threshold'] == threshold)][column]
            
            if len(data) > 0:
                is_normal, p_val = check_normality(data.values, f"{format_name}, Noise {noise}")
                normality_results[(format_name, noise)] = is_normal
                if not is_normal:
                    all_normal = False
    ...
    for format_name, (column, threshold) in formats.items():
        ...
        for noise in noise_values:
            ...
            if len(data) > 0:
                ...
                print(f"WARNING: No data for {format_name}, Noise {noise}")
    ...
    # Set up x values and colors
    x = np.arange(len(noise_values))
    colors = plt.cm.tab10(np.linspace(0, 1, len(formats)))
    
    for i, (format_name, format_data) in enumerate(results.items()):
        # Plot line with error bars
        plt.errorbar(x, format_data['central_values'], 
                    yerr=[format_data['ci_lows'], format_data['ci_highs']], 
                    label=format_name, color=colors[i], 
    ...
    plt.title(f'Storage Format Comparison vs Noise Level\n({statistic_used})', fontsize=14)
    ...
    # Save plot
    plt.savefig('/users/nrottste/HBDIA/benchmarking/Plotting/format_comparison.pdf', 
    ...
    for format_name in formats.keys():
        all_values = [val for val in results[format_name]['central_values'] if not np.isnan(val)]
        if all_values:
            if use_mean:
                central_summary = np.mean(all_values)
            else:
                central_summary = np.median(all_values)
            print(f"{format_name}: {statistic_name}={central_summary:.2f}, "
